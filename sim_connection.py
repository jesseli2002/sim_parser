...
class SimConnection(Connection):
    ...
    def _rocket_handshake(self):
        ...
        input(f"Received SYN; press enter to continue. PID={self.rocket.pid}\n")
    ...
    def send(self, device_address, data):
        if device_address != self.device_address:
            raise Exception(
                f"Connection does not support device_address={device_address}"
            )
        self.broadcast(data)
    ...
    def _find_executable(self, executable_name):
        ...
        neighbour_build_file_exists = os.path.exists(
            os.path.join(flare_path, neighbour_build_file)
        )
